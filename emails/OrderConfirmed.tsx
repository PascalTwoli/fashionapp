...
export default function OrderConfirmed(props: BaseOrderProps) {
  const { order_number, items, subtotal, shipping_fee, total_amount } = props;
  return (
    <EmailLayout preview={`Your order ${order_number} has been confirmed.`}>
      <Badge label="Order Confirmed" />
      <Text style={{ fontSize: 20, fontWeight: 700, marginTop: 18 }}>Your order is confirmed!</Text>
      <Text style={{ fontSize: 13, color: colors.muted, marginTop: 4 }}>
        Order <strong>{order_number}</strong> has been confirmed and will soon move to processing.
      </Text>
      <Divider />
