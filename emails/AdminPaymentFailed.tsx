...
export default function AdminPaymentFailed(props: BaseOrderProps) {
  const { order_number, payment_error, items, subtotal, shipping_fee, total_amount } = props;
  return (
    <EmailLayout preview={`Payment failed for order ${order_number}.`}>
      <Badge label="Payment Failed" color={colors.danger} />
      <Text style={{ fontSize: 18, fontWeight: 700, marginTop: 18 }}>Payment failed</Text>
      <Text style={{ fontSize: 13, color: colors.muted, marginTop: 4 }}>
        Payment for order <strong>{order_number}</strong> was not completed.
      </Text>
      {payment_error && (
...
AdminPaymentFailed.PreviewProps = {
